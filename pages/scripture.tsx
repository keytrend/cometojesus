...
export default function Scripture() {
  ...
  // 페이지 최초 로드 시 guest_user_id 초기화 (없으면 UUID 생성)
  useEffect(() => {
  ...
  // ① URL canon 파라미터 → canonId 동기화
  useEffect(() => {
    if (router.query.canon) setCanonId(router.query.canon as string)
  }, [router.query.canon])

  // ② books 로드 후 URL 파라미터로 자동 이동
  useEffect(() => {
    if (!router.isReady || autoNavigated.current || !books.length) return
    const bookParam    = router.query.book    as string | undefined
    const chapterParam = router.query.chapter as string | undefined
    if (!bookParam || !chapterParam) return

    const found = books.find(b =>
      b.name_ko === bookParam || b.name_en === bookParam
    )
    if (found) {
      ...
      loadChapter(found, chNum)
    }
  }, [router.isReady, books, router.query.book, router.query.chapter])

  // ③ canonId 변경 시 books 재로드
  useEffect(() => {
  ...
  // ④ URL section 파라미터 → activeSection 동기화
  useEffect(() => {
