...
// /api/search 는 canon name_ko를 반환하므로 canon_id로 매핑
const CANON_ID: Record<string, string> = {
  '구약전서':    'ot',
  '신약전서':    'nt',
  '몰몬경':      'bom',
  '교리와 성약': 'dc',
      ...
      <div className="landing-hero">
        ...
        <div style={{ marginTop: '1.5rem', width: '100%', display: 'flex', justifyContent: 'center' }}>
          <SearchBar
            lang={lang}
            onSelect={result => {
              const canonId = CANON_ID[result.canon] ?? 'ot'
              window.location.href = `/scripture?canon=${canonId}&book=${encodeURIComponent(result.book)}&chapter=${result.chapter_num}&section=s4`
            }}
          />
        </div>
      </div>
